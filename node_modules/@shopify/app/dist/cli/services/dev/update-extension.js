...
import { errorsToString as zodErrorsToString } from '@shopify/cli-kit/node/schema';
export async function updateExtensionDraft({ extension, developerPlatformClient, apiKey, registrationId, stdout, stderr, }) {
    ...
    if (mutationResult.extensionUpdateDraft?.userErrors?.length > 0) {
        ...
        if (draftUpdateSuccesMessage)
            outputInfo(draftUpdateSuccesMessage, stdout);
    }
}
export async function reloadExtensionConfig({ extension }) {
    const abort = (errorMessage, _fallbackOutput, _path, rawErrors) => {
        let message = typeof errorMessage === 'string' ? errorMessage : errorMessage.value;
        if (rawErrors)
            message = zodErrorsToString(rawErrors);
  ...
  - Handles are immutable, you can't change them once they are set.`);
    ...
    const newConfig = await parseConfigurationObject(extension.specification.schema, extension.configurationPath, configObject, abort);
