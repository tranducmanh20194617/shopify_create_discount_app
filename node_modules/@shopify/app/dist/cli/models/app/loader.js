...
export function parseHumanReadableError(issues) {
...
export async function parseConfigurationObject(schema, filepath, configurationObject, abortOrReport) {
    ...
    if (!parseResult.success) {
        return abortOrReport(outputContent `App configuration is not valid\nValidation errors in ${outputToken.path(filepath)}:\n\n${parseHumanReadableError(parseResult.error.issues)}`, fallbackOutput, filepath, parseResult.error.issues);
    }
...
class AppLoader {
    constructor({ directory, configName, mode, specifications, remoteFlags }) {
    ...
    async loaded() {
        ...
        const { webs, usedCustomLayout: usedCustomLayoutForWeb } = await this.loadWebs(directory, configuration.web_directories);
    ...
    async createConfigExtensionInstances(directory, appConfiguration) {
        return this.specifications
            ...
            if (Object.keys(specConfiguration).length === 0)
                return;
...
class AppConfigurationLoader {
    constructor({ directory, configName, specifications, remoteFlags }) {
        this.directory = directory;
        this.configName = configName;
        this.specifications = specifications;
        this.remoteFlags = remoteFlags ?? [];
