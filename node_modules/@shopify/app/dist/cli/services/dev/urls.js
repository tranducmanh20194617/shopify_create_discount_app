...
export async function generateFrontendURL(options) {
    ...
    if (isSpin() && !options.tunnelUrl) {
        ...
        const cliPortManualExecution = await fetchSpinPort(spinVariables.partnersSpinService, spinVariables.manualCliSpinPortName);
        ...
        throw new AbortError(`Error building cli url in spin, cli as service port: ${cliPortProcfileExecution}, manual cli port: ${cliPortManualExecution}`);
    }
...
export async function updateURLs(urls, apiKey, developerPlatformClient, localApp) {
    const variables = { apiKey, ...urls };
    const result = await developerPlatformClient.updateURLs(variables);
