...
export async function uploadExtensionsBundle(options) {
    ...
    const result = await handlePartnersErrors(() => options.developerPlatformClient.deploy(variables));
 ...
 */
export async function getExtensionUploadURL(developerPlatformClient, apiKey) {
    const variables = {
        apiKey,
        bundleFormat: 1,
    };
    const result = await handlePartnersErrors(() => developerPlatformClient.generateSignedUploadUrl(variables));
...
export async function uploadWasmBlob(extensionIdentifier, wasmPath, developerPlatformClient) {
...
async function getFunctionExtensionUploadUrlFromPartners(developerPlatformClient) {
