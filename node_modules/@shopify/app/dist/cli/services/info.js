        ...
        if ('specifications' in appWithSupportedExtensions) {
            appWithSupportedExtensions = {
                ...
                specifications: appWithSupportedExtensions.specifications?.map((spec) => {
                    ...
                    return objectWithoutSchema(spec);
                }),
            };
        }
        return outputContent `${JSON.stringify(Object.fromEntries(Object.entries(appWithSupportedExtensions).filter(([key]) => key !== 'configSchema')), null, 2)}`;
    }
    else {
...
function objectWithoutSchema(obj) {
    const { schema, ...rest } = obj;
    return rest;
}
function withPurgedSchemas(extensions) {
