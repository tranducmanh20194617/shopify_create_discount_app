...
export async function migrateFlowExtensions(extensionsToMigrate, appId, remoteExtensions, developerPlatformClient) {
    const migratedIDs = await Promise.all(extensionsToMigrate.map(({ remote }) => migrateFlowExtension(appId, remote.id, developerPlatformClient)));
    const typesMap = new Map([
...
export async function migrateFlowExtension(apiKey, registrationId, developerPlatformClient) {
    const variables = {
        apiKey,
        registrationId,
    };
    const result = await developerPlatformClient.migrateFlowExtension(variables);
